<?php
	...
	if($op=='card_edit'){
		$card = pdo_fetch("SELECT * FROM " . tablename('card') . " where uid = :uid and :uniacid", array(':uid' => $uid,':uniacid' => $_W['uniacid']));
		if($_W['ispost']){
			$data = array(
			    'uid'   =>   $uid,
				'uniacid'  => $_W['uniacid'],
			...
			if($card){
				...
				$res = pdo_update('card' ,$data,array('uniacid' => $_W['uniacid'],'uid' => $uid));
			} else {
			...
			//if($res){
	            ...
	            if(!$card){
		            message("名片未生成", $this->createMobileUrl('active'));
	            }
                ...
                imagealphablending($xgm,false);
              	...
              	if($card['baseimage']){
	             	...
	             	if($card['model'] == '4'){
	             		$logo = $_W['siteroot'] . 'addons/card_jpg/data/color/'.$colortype.'_400.png';
	            	} else {
	            		$logo = $_W['siteroot'] . 'addons/card_jpg/data/color/'.$colortype.'_720.png';
	              	}
               	}
                $bg = imagecreatefrompng($logo);
                $img_info = getimagesize($logo);
                $img_w = $img_info[0];
                $img_h = $img_info[1];
		        ...
		        $font = '../addons/card_jpg/data/font/'.$fonttype.'.ttf'; //文字合成
		        $model = $card['model']?$card['model']:'1';
		        switch ($model){
		            case '1':
		        	    imagecopymerge($im, $xgm, 42, 30, 0, 0, imagesx($xgm),imagesy($xgm), 100); //头像合成
		                imagecopymerge($im, $qrim, 59, 235, 0, 0, imagesx($qrim),imagesy($qrim), 100); //二维码合成
				        ...
				        if($card['address']){
		  	            ...
		  	            break;
			        case '2':
			            imagecopymerge($im, $xgm, 30, 37, 0, 0, imagesx($xgm),imagesy($xgm), 100); //头像合成
	     	            imagecopymerge($im, $qrim, 522, 40, 0, 0, imagesx($qrim),imagesy($qrim), 100); //二维码合成
			        ...
			        case '3':
		    	        ...
		    	        break;
			        case '4':
		    	        imagecopymerge($im, $xgm, 110, 55, 0, 0, imagesx($xgm),imagesy($xgm), 100); //头像合成
	        	        imagecopymerge($im, $qrim, 230, 264, 0, 0, imagesx($qrim),imagesy($qrim), 100); //二维码合成
	    	    ...
	    	    imagedestroy($im);
	    	    message("操作成功", $this->createMobileUrl('active'));
			//} else {
				//message("生成名片",$this->createMobileUrl('card_createimg',array('uid'=>$uid)));
			//}
		}
	}
	
    include $this->template('card_edit');
